...
# Helper function to clean-up the terminal so things don't get messy
...
def book_exists_in_personal_list(title, author):
    ...
    for row in personal_books_data[1:]:  # Skip header row
        if (row[1].lower() == title.lower() and
           row[2].lower() == author.lower()):
            return True
    return False
...
def search_books():
    ...
    while True:
        ...
        # Loop until until user no longer wants to add any books anymore
                ...
                try:
                    ...
                    if 1 <= book_choice <= len(dictionary_books
                        ...
                        while True:
                            ...
                            if add_another == '':
                                clear()
                                return  # Exit the function if the user
                                # doesn't want to add another book
                            elif add_another == 'y':
                                clear()
                                get_books_in_category(category_choice,
                                                      categories,
                                                      dictionary_books)
                                break
                            else:
        ...
        if choice == '1':
            ...
            while True:
                ...
                if choice == 'd':
                    ...
                    if deleted:
                        continue  # Refresh personal_list worksheet
                    else:
                        break
                elif choice == '':
                    clear()
